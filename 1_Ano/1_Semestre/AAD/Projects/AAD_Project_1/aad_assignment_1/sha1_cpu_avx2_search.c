/*
 * sha1_cpu_avx2_search.c
 *
 * AVX2-based CPU search for DETI coins (8 lanes).
 * Uses sha1_avx2() and save_coin() vault.
 *
 * Prints progress every 50 million attempts and coin details when found.
 *
 * Build: cc -march=native -mavx2 -O3 -Wall -Wshadow -Werror sha1_cpu_avx2_search.c aad_sha1_cpu.h aad_vault.h aad_utilities.h aad_data_types.h -o sha1_cpu_avx2_search
 */ 
 ...
 int main(void){
     ...
     // Data structures for AVX2 lanes
     ...
     while(keep_running){
         ...
         for(int lane=0; lane<N_LANES; ++lane){
             ...
             for (int j=0; j<12; ++j) data[lane].c[j^3] = (u08_t)prefix[j];
             data[lane].c[54^3] = (u08_t)'\n';
             data[lane].c[55^3] = 0x80u;
             // Fill bytes 12..53
             for(unsigned i=12;i<=53;i++){
                 if(i<20) data[lane].c[i^3] = counter.c[i-12];
                 else data[lane].c[i^3] = (u08_t)(0x20 + ((i*17 + attempts + lane) % (0x7E - 0x20 +1)));
             }
         }
         ...
         for(int lane=0; lane<N_LANES; ++lane){
             if(interleaved_hash[0][lane] == 0xAAD20250u){
                 u32_t hash_lane[5];
                 for(int w=0; w<5; w++) hash_lane[w] = interleaved_hash[w][lane];
